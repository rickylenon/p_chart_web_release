...
const OUTPUT_FILE = path.join(process.cwd(), "data", "production-data-latest.sql");
...
function escapeSqlValue(value) {
  ...
  if (value && typeof value === 'object' && value.toString && !Array.isArray(value) && !(value instanceof Date)) {
...
function generateInsertStatement(tableName, records, columns) {
  ...
  const updateColumns = columns.filter(col => col !== 'id').map(col => {
...
async function exportUsers() {
  try {
    ...
    const columns = ['id', 'username', 'password', 'name', 'email', 'role', 'isActive', 'createdAt', 'lastLogin', 'department'];
...
async function exportMasterDefects() {
  try {
    ...
    const columns = ['id', 'name', 'description', 'category', 'applicableOperation', 'reworkable', 'machine', 'isActive', 'deactivatedAt', 'deactivatedById'];
...
async function exportProductionOrders() {
  try {
    ...
    const columns = ['id', 'poNumber', 'lotNumber', 'poQuantity', 'itemName', 'status', 'currentOperation', 'currentOperationStartTime', 'currentOperationEndTime', 'createdAt', 'updatedAt', 'editingUserId', 'editingUserName', 'lockedAt', 'costPerUnit', 'totalDefectCost', 'lastCostUpdate'];
...
async function exportOperations() {
  try {
    ...
    const columns = ['id', 'productionOrderId', 'operation', 'operatorId', 'startTime', 'endTime', 'inputQuantity', 'outputQuantity', 'productionHours', 'accumulatedManHours', 'rf', 'lineNo', 'shift', 'encodedById', 'encodedTime', 'defectCost'];
...
async function exportOperationDefects() {
  try {
    ...
    const columns = ['id', 'operationId', 'defect_id', 'defectName', 'defectCategory', 'defectMachine', 'defectReworkable', 'quantity', 'quantityRework', 'quantityNogood', 'quantityReplacement', 'recordedAt', 'recordedById'];
    const records = defects.map(defect => ({
      id: defect.id,
...
async function exportStandardCosts() {
  try {
    ...
    try {
      ...
      log("Could not check if standard_costs table exists, proceeding with export attempt", "WARN");
    }
    ...
    const columns = ['id', 'itemName', 'description', 'costPerUnit', 'currency', 'isActive', 'createdAt', 'updatedAt', 'createdById', 'updatedById'];
    ...
    if (error.code === 'P2021' || error.message.includes('does not exist')) {
      ...
      log("Please run: npx prisma db push  or  npx prisma migrate deploy", "ERROR");
...
async function exportOperationDefectEditRequests() {
  try {
    if (shouldSkipTable('operation_defect_edit_requests')) {
      log("Skipping operation defect edit requests export (--skip-edit-requests)");
    ...
    const columns = ['id', 'operationDefectId', 'operationId', 'productionOrderId', 'requestedById', 'requestType', 'defectId', 'defectName', 'defectCategory', 'defectReworkable', 'defectMachine', 'currentQty', 'currentRw', 'currentNg', 'currentReplacement', 'requestedQty', 'requestedRw', 'requestedNg', 'requestedReplacement', 'operationCode', 'reason', 'status', 'resolvedById', 'resolutionNote', 'createdAt', 'resolvedAt'];
    ...
    return generateInsertStatement('operation_defect_edit_requests', records, columns);
  } catch (error) {
...
async function exportNotifications() {
  try {
    if (shouldSkipTable('notifications')) {
      log("Skipping notifications export (--skip-notifications or --fast mode)");
    ...
    const columns = ['id', 'type', 'title', 'message', 'isRead', 'createdAt', 'updatedAt', 'linkUrl', 'userId', 'sourceId', 'sourceType', 'metadata'];
...
async function exportAuditLogs() {
  try {
    ...
    const columns = ['id', 'tableName', 'recordId', 'action', 'oldValues', 'newValues', 'userId', 'timestamp', 'ipAddress', 'userAgent'];
...
async function exportSessions() {
  try {
    ...
    const columns = ['id', 'userId', 'token', 'ipAddress', 'userAgent', 'createdAt', 'expiresAt', 'lastActivity'];
...
async function exportProductionData() {
  ...
  try {
    ...
    sqlContent += `-- Source Database: ${NEON_DATABASE_URL.replace(/password=[^@]+/, 'password=***')}\n`;
    sqlContent += `\n-- BEGIN TRANSACTION\n`;
    sqlContent += `BEGIN;\n\n`;
    ...
    const allTables = ['users', 'master_defects', 'production_orders', 'operations', 'operation_defects', 'operation_steps', 'operation_lines', 'standard_costs', 'operation_defect_edit_requests', 'audit_logs', 'sessions'];
