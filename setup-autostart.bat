...
echo   START_SCRIPT=%START_SCRIPT% >> "%LOG_FILE%"
echo   TASK_XML=%TASK_XML% >> "%LOG_FILE%"

:: Check if start.bat exists
echo Checking for start.bat... >> "%LOG_FILE%"
if not exist "%START_SCRIPT%" (
    echo ERROR: start.bat not found at %START_SCRIPT% >> "%LOG_FILE%"
    echo Error: start.bat not found at %START_SCRIPT%
    echo Check %LOG_FILE% for details.
    pause
    exit /b 1
)
echo start.bat found >> "%LOG_FILE%"
...
echo     ^<Exec^> >> "%TASK_XML%"
echo       ^<Command^>%START_SCRIPT%^</Command^> >> "%TASK_XML%"
