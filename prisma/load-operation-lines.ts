...
async function loadOperationLines() {
  console.log("Starting to load operation lines data...");
  ...
  // Clear existing data
  console.log("Clearing existing operation lines...");
  await prisma.operationLine.deleteMany({});

  // Insert operation lines
  console.log("Inserting operation lines...");
  let inserted = 0;
  let errors = 0;

  for (const line of operationLines) {
    try {
      await prisma.operationLine.create({
        data: line,
      });
      inserted++;
      ...
      if (inserted % 20 === 0) {
        console.log(`Inserted ${inserted} operation lines`);
      }
    } catch (error) {
      console.error(
  ...
  console.log(
    `Operation lines data loaded: ${inserted} records inserted, ${errors} errors`
  );
}
